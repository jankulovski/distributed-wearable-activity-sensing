...
void mpu9250_read_acc(void *pvParameters){
	...
	while(1){
		while(x < 10){
			...
			accelerations[x] = sqrt_((x_acc_g * x_acc_g)+(y_acc_g * y_acc_g)+ (z_acc_g * z_acc_g) );

			vTaskDelay(50 / portTICK_PERIOD_MS); // sleep 100ms
			x++;
		}
		...
		// zero-reset accelerations array
...
void user_init(void)
{
    ...
    char ssid[] = "Ahmed's iPhone";
    char password[] = "ahmed5592";
    ...
    // board ip
	IP4_ADDR(&info.ip, 172, 20, 10, 11);
	// gateway ip
	IP4_ADDR(&info.gw, 172, 20, 10, 0);
	IP4_ADDR(&info.netmask, 255, 255, 255, 240);
